...
(function($) {
	...
	$(function() {
		...
		// Main Sections: Three.
			...
			// Lightbox gallery.
				$('#three').poptrox({
					caption: function($a) { return $a.next('.caption-src').html() },
					overlayColor: '#2c2c2c',
					overlayOpacity: 0.85,
					popupCloserText: '',
					popupLoaderText: '',
					...
					usePopupNav: false,
					windowMargin: (skel.isActive('small') ? 0 : 20),
					onPopupClose: () => {
						document.querySelector('.caption').innerHTML = ""
					}
				});

		// enable click on Lightbox caption
		$('.poptrox-popup>.caption').on('click',(evt) => {
			evt.stopPropagation()
		})
